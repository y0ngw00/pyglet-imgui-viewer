...
#
# based on https://github.com/simonalexanderson/ListenDenoiseAction
#
##############################
...
class LoggingMixin:
    ...
    def log_results(self, pred_clips, file_name, log_prefix, logdir=None, render_video=True):
        ...
        if logdir is None:
            # logdir = f"{self.logger.save_dir}/{self.logger.name}/wandb/run-{self.logger.version}/"
            logdir = self.logger.experiment.dir
    ...
    def feats_to_bvh(self, pred_clips):
        ...
        data_pipeline = jl.load(Path(self.hparams.Data['dataset_root']) / self.hparams.Data["datapipe_filename"])
