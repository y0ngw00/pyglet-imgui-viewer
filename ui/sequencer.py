...
class Sequencer:
    ...
    def on_mouse_drag(self, x, y, dx, dy):
        if self.is_mouse_in_frame_bar(x,y):
            frame = self.parent_window.get_frame()
...
class Sequence:
    def __init__(self, target,sequence_pos_start,sequence_height):
        ...
        if len(target.joints) > 0 and len(target.joints[0].positions)>0:
            self.tracks.append(SequenceTrack(self, target.name, 0, len(target.joints[0].positions)))
    ...
    def render(self, idx, is_picked):
        ...
        layout_padding = [10,10]
        self.x_origin = canvas_pos.x+layout_padding[0]
        self.y_origin = canvas_pos.y+layout_padding[1] + idx * self.sequence_height
        
        if is_picked:
            draw_list.add_rect_filled(canvas_pos.x+layout_padding[0], self.y_origin+self.padding_y, 
                        self.x_origin+self.xsize_box, self.y_origin+self.ysize_box-self.padding_y, 
                        self.background_color, rounding=4)
        
        draw_list.add_rect(self.x_origin+self.sequence_pos_start, self.y_origin+self.padding_y, 
                           self.x_origin+self.xsize_box-5, self.y_origin+self.ysize_box-self.padding_y, 
                           self.sequence_color, rounding=4, thickness=2)
   
        for track in self.tracks:
            track.render(self.x_origin+self.sequence_pos_start, self.y_origin+self.padding_y, dx=1, dy = self.ysize_box-2*self.padding_y)
    ...
    def insert_track(self, file_path, start_frame):
        ...
        self.tracks.append(SequenceTrack(self, name, start_frame, start_frame + len(joints[0].positions)))
...
class SequenceTrack:
    ...
    def render(self, x, y, dx, dy):
        draw_list = imgui.get_window_draw_list()
        draw_list.add_rect_filled(x + dx * self.frame_start, y, 
                           x+dx * self.frame_end, y+dy, 
                           self.track_color, rounding=4)
        
        draw_list.add_text(x + dx * self.frame_start, y, self.text_color,self.name)
